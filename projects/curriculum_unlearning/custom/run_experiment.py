...
# --- run_curriculum.py의 _delta_metrics, ci95, retrain_cache_path 함수는 그대로 가져와 사용 ---
...
def run_experiment():
    ...
    # 분할 시에는 원본 모델의 representation이 필요할 수 있으므로 모델을 전달
    forget_partitions = partition_forget_set(total_forget_indices, train_eval_ds, orig_model, CONFIG)

    # 4. 실험할 언러닝 기법 목록 정의
    unlearning_methods = {
        "Finetune": unlearn_finetune,
        "L1-sparse": unlearn_l1_sparse,
        "NegGrad": unlearn_neggrad,
        "NegGrad+": unlearn_neggrad_plus,
        "Random-Label": unlearn_random_label_fixed,
        "SalUn": unlearn_salun_fixed,
        "SCRUB": unlearn_scrub
    }
    ...
    for method_name, unlearn_fn in unlearning_methods.items():
        ...
        for stage, S_forget in enumerate(forget_partitions, start=1):
            ...
            # 2. 로더 및 데이터셋 Subset 구성
            # methods.py에서 Subset 객체가 직접 필요한 경우를 대비
            retain_set_obj = Subset(train_ds, retain_idx)
            forget_set_obj = Subset(train_ds, forget_idx)
            
            r_train, f_train, r_eval, f_eval = build_loaders(
                train_ds, train_eval_ds, retain_idx, forget_idx, bs, shuffle_train=True
            )
            
            # 언러닝 함수에 전달할 loaders 딕셔너리
            loaders = {
                "retain_train": r_train, "forget_train": f_train,
                "retain_eval": r_eval, "forget_eval": f_eval,
                "retain_set_obj": retain_set_obj, "forget_set_obj": forget_set_obj
            }
            ...
            # 4. 언러닝 적용 (누적된 모델에 추가로 언러닝)
            print(f"[UNLEARN] {method_name} stage {stage} (|forget_added|={len(S_forget)}, |forget_total|={len(cumulative_forget_indices)})")
            # model_u는 이전 스테이지에서 언러닝된 모델. 이 모델을 계속 업데이트함.
